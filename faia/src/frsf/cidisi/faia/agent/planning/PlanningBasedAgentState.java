...
public abstract class PlanningBasedAgentState extends AgentState {
	...
	public Hashtable<?, ?>[] query(String query) {
		Map<String, ?>[] mapas = this.prologConnector.query(this.prepareQuery(query));

		Hashtable<?, ?>[] result = new Hashtable[mapas.length];
		for(int i = 0; i < mapas.length; i++){
			result[i] = new Hashtable<>(mapas[i]);
		}
		return result;
	}

	public Hashtable<?, ?>[] plainQuery(String query) {
		Map<String, ?>[] mapas = this.prologConnector.query(query);
